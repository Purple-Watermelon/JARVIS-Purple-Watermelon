...
export default function App() {
  ...
  // 일반 데이터 저장
  // ready가 true일 때만 저장해서, Firebase 데이터를 불러오기 전에
  // 기본값이 기존 데이터를 덮어쓰는 사고를 방지한다.
  const save = async (key, value) => {
    try {
      await setDoc(doc(db, key, USER_ID), { value });
    } catch(e) {
  ...
  // Diary 전용 저장
  // ------------------------------------------------------------
  // 전체 diaryData를 Firebase에 덮어쓰지 않고,
  // 수정한 날짜 하나만 업데이트한다.
  //
  // 예:
  // value.2026-09-01 만 변경
  // value.2026-08-06 등 다른 날짜는 건드리지 않는다.
  // ------------------------------------------------------------
  const saveDiaryEntry = async (dateKey, entry) => {
    if (!ready) {
      throw new Error('아직 Firebase 데이터를 불러오는 중입니다.');
    }

    if (!dateKey) {
      throw new Error('일기 날짜가 없습니다.');
    }

    try {
      const diaryRef = doc(db, 'diary', USER_ID);
      ...
      // Firebase 저장이 성공한 뒤에만 화면 상태도 갱신
      setDiaryData(prev => ({
        ...prev,
        [dateKey]: entry
      }));

      console.log(`✅ Diary 저장 완료: ${dateKey}`);
    } catch (error) {
      console.error(`❌ Diary 저장 실패: ${dateKey}`, error);
      throw error;
    }
  };
  ...
  // ⚠️ Diary 전체 자동 저장은 제거했다.
  // DiaryTab에서 saveDiaryEntry()를 통해 해당 날짜만 저장한다.
  ...
  if (loadError) return (
    <div style={{
      ...
      <div style={{
        ...
        기록을 보호하기 위해 저장을 멈췄어요.<br/>
        인터넷 연결을 확인하고 잠시 후 새로고침 해주세요.
      </div>
  ...
  return (
    <div style={{
      ...
      color:'var(--text)',
      position:'relative',
      boxShadow:'0 0 40px rgba(124,92,191,0.15)'
    }}>
      ...
      <div style={{ paddingBottom: 70 }}>
        ...
        {tab === 'diary' &&
          <DiaryTab
            ...
            unlocked={diaryUnlocked}
            setUnlocked={setDiaryUnlocked}
          />
        }
