...
const DEFAULT_UNITS = ['개', 'ml', 'g', 'L', '병', '팩', '롤', '박스'];
...
// ── 꺾은선 그래프 (SVG) ────────────────────────────────────────────────
function LineChart({ history, unit }) {
  if (!history || history.length < 2) return null;
  
  const sorted = [...history].sort((a, b) => new Date(a.date) - new Date(b.date));
  const data = sorted.map(h => ({
    date: h.date,
    price: h.qty ? Math.round(h.totalAmount / h.qty) : 0,
  }));
  ...
  const xStep = graphW / (data.length - 1 || 1);
  const points = data.map((d, i) => {
    const x = padding.left + i * xStep;
    const y = padding.top + graphH - ((d.price - minPrice) / range) * graphH;
    return { x, y, ...d };
  });
    ...
    <div style={{ overflowX: 'auto', marginBottom: 16 }}>
      <svg width={width} height={height} style={{ minWidth: width, display: 'block' }}>
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((ratio, i) => {
          ...
          return (
            <line key={i} x1={padding.left - 4} y1={y} x2={width - padding.right} y2={y} stroke="var(--border)" strokeWidth="0.5" opacity={ratio === 0 || ratio === 1 ? 1 : 0.3} strokeDasharray="2" />
          );
        })}
        
        {/* Y축 라벨 */}
        <text x={padding.left - 8} y={padding.top + 4} textAnchor="end" fontSize="10" fill="var(--sub)">{fmt(maxPrice)}</text>
        <text x={padding.left - 8} y={height - padding.bottom + 4} textAnchor="end" fontSize="10" fill="var(--sub)">{fmt(minPrice)}</text>
        ...
        {/* 데이터 포인트 */}
        {points.map((p, i) => (
          <circle key={i} cx={p.x} cy={p.y} r="3" fill="var(--accent)" stroke="var(--card)" strokeWidth="2" />
        ))}
        
        {/* X축 라벨 (첫 중간 마지막) */}
        {points.map((p, i) => {
          if (i !== 0 && i !== Math.floor(points.length / 2) && i !== points.length - 1) return null;
          const dateStr = p.date.slice(-2);
          return (
            <text key={`x${i}`} x={p.x} y={height - padding.bottom + 16} textAnchor="middle" fontSize="10" fill="var(--sub)">
              {p.date.slice(5, 7)}/{dateStr}
            </text>
          );
        })}
      </svg>
...
// ── 아이템 상세 (하단 시트) ──────────────────────────────────────────────
function ItemDetail({ item, onClose, onAddHistory, onEditHistory, onDelHistory, onToggleNotify, onUpdateUnit }) {
  ...
  const openAdd = () => { setEditH(null); setHForm({ date: today, qty: '' }); setShowForm(true); };
  const openEdit = h => { setEditH(h); setHForm({ date: h.date, qty: h.qty, totalAmount: h.totalAmount, memo: h.memo || '', store: h.store || '' }); setShowForm(true); };

  const save = () => {
    if (!hForm.date || !hForm.qty || !hForm.totalAmount) return;
    const entry = {
      id: editH?.id || uid(),
      date: hForm.date,
      totalAmount: Number(hForm.totalAmount),
      qty: Number(hForm.qty),
      unit: unit,
      store: hForm.store || '',
      memo: hForm.memo || '',
    };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(26,20,40,0.5)', zIndex: 200, display: 'flex', flexDirection: 'column' }} onClick={onClose}>
      <div style={{ flex: 1 }} />
      <div style={{ background: 'var(--card)', borderRadius: '22px 22px 0 0', maxHeight: '90vh', display: 'flex', flexDirection: 'column', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding: '0 20px', marginBottom: 12 }}>
          <div style={{ background: 'var(--bg)', borderRadius: 12, padding: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
            <div>
              <div style={{ fontSize: 11, color: 'var(--sub)', fontWeight: 600 }}>📏 기본 단위</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--accent)' }}>{unit}</div>
            </div>
            <button onClick={() => { setShowUnitEdit(true); setUnitForm(unit); }} style={{
        ...
        {showUnitEdit && (
          <div style={{ padding: '0 20px', marginBottom: 12 }}>
            <div style={{ background: 'var(--bg)', borderRadius: 12, padding: 12 }}>
              <div style={{ marginBottom: 8 }}><label style={lbl}>단위</label></div>
              <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                <input style={inp} placeholder="예: ml, g, 개" value={unitForm} onChange={e => setUnitForm(e.target.value)} />
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => setShowUnitEdit(false)} style={{ flex: 1, padding: '8px', borderRadius: 8, border: '1.5px solid var(--border)', color: 'var(--text)', background: 'transparent', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>취소</button>
                <button onClick={saveUnit} style={{ flex: 1, padding: '8px', borderRadius: 8, background: 'var(--accent)', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 700 }}>적용</button>
              </div>
            </div>
          </div>
        )}
        ...
        {history.length >= 2 && (
          <div style={{ padding: '0 20px', marginBottom: 12 }}>
            <div style={{ fontSize: 11, color: 'var(--sub)', fontWeight: 600, marginBottom: 8 }}>📊 단위당 가격 추이</div>
            <div style={{ background: 'var(--bg)', borderRadius: 12, padding: 12 }}>
              <LineChart history={history} unit={unit} />
            </div>
          </div>
        )}
        ...
        {history.length > 0 && (
          <div style={{ padding: '0 20px', marginBottom: 12 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              ...
              {interval && (
                <div style={{ background: 'var(--bg)', borderRadius: 12, padding: 12 }}>
                  <div style={{ fontSize: 10, color: 'var(--sub)', fontWeight: 600, marginBottom: 4 }}>🔄 구매 주기</div>
                  <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--text)' }}>{interval}일</div>
                </div>
              )}
            </div>
          </div>
          ...
          <div style={{ padding: '0 20px', marginBottom: 12 }}>
            <div style={{ background: 'var(--bg)', borderRadius: 14, padding: 14 }}>
              ...
              <div style={{ marginBottom: 10 }}>
                <label style={lbl}>수량 ({unit}) *</label>
                <input style={inp} inputMode="numeric" placeholder="0" value={hForm.qty||''} onChange={e => HF('qty', e.target.value)} />
              </div>
              <div style={{ marginBottom: 10 }}>
              ...
              <div style={{ marginBottom: 10 }}><label style={lbl}>메모</label><input style={inp} placeholder="참고 사항" value={hForm.memo||''} onChange={e => HF('memo', e.target.value)} /></div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => { setShowForm(false); setEditH(null); setHForm({}); }} style={{ flex: 1, padding: '11px', borderRadius: 12, border: '1.5px solid var(--border)', color: 'var(--text)', background: 'transparent', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>취소</button>
                <SaveBtn onClick={save} disabled={!hForm.date || !hForm.qty || !hForm.totalAmount} label={editH ? '수정' : '저장'} />
              </div>
            </div>
          </div>
        ) : (
          <div style={{ padding: '0 20px', marginBottom: 16 }}>
            <button onClick={openAdd} style={{ width: '100%', borderRadius: 12, padding: '12px', background: 'var(--accent)', color: '#fff', fontWeight: 700, fontSize: 14, border: 'none', cursor: 'pointer' }}>+ 추가 기록</button>
          </div>
        )}

        {/* 기록 목록 */}
        <div style={{ padding: '0 20px 40px', flexGrow: 1 }}>
          {history.length === 0
                ...
                return (
                  <div key={h.id} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                      ...
                      <div style={{ flex: 1 }} />
                      <button onClick={() => openEdit(h)} style={{ fontSize: 12, padding: '3px 8px', borderRadius: 6, border: '1px solid var(--border)', color: 'var(--accent)', background: 'transparent', cursor: 'pointer' }}>수정</button>
                      <button onClick={() => onDelHistory(h.id)} style={{ fontSize: 12, padding: '3px 8px', borderRadius: 6, border: '1px solid var(--red-bg)', color: 'var(--red)', background: 'transparent', cursor: 'pointer' }}>삭제</button>
                    </div>
                    <div style={{ fontSize: 13.5, fontWeight: 700, color: 'var(--text)' }}>
                      {h.qty}{h.unit} · {fmt(h.totalAmount)}원
                      {unitPrice !== null && (
                        <span style={{ color: isMin ? 'var(--green)' : 'var(--sub)', fontWeight: 600 }}> · {fmt(unitPrice)}원/{h.unit}</span>
                      )}
                    </div>
...
export default function CycleTab({ data, setData }) {
  ...
  const saveItem = () => {
    ...
    if (editItem) {
      setData(p => p.map(e => e.id === editItem.id ? { ...e, name: form.name.trim(), category: form.category||'', memo: form.memo || '', unit: form.unit || '개' } : e));
    } else {
      setData(p => [...p, { id: uid(), name: form.name.trim(), category: form.category||'', memo: form.memo || '', unit: form.unit || '개', notifyOff: false, history: [] }]);
    }
  ...
  const updateUnit = (itemId, newUnit) =>
    setData(p => p.map(e => e.id === itemId ? { ...e, unit: newUnit } : e));
  ...
  return (
    <div style={{ padding: 16, paddingBottom: 90 }}>
          ...
          <div key={item.id} onClick={() => setDetail(item)} style={{ background: 'var(--card)', borderRadius: 16, padding: 16, marginBottom: 10, cursor: 'pointer', border: '1.5px solid var(--border)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 10, color: 'var(--accent)', fontWeight: 600, marginTop: 3 }}>📏 {item.unit || '개'}</div>
              ...
              <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                <button onClick={e => { e.stopPropagation(); setEditItem(item); setForm({ name: item.name, category: item.category||'', memo: item.memo || '', unit: item.unit || '개' }); setShowItemModal(true); }} style={{ fontSize: 12, padding: '5px 10px', borderRadius: 8, border: '1px solid var(--border)', color: 'var(--accent)', background: 'transparent', cursor: 'pointer' }}>수정</button>
                <button onClick={e => { e.stopPropagation(); delItem(item.id); }} style={{ fontSize: 12, padding: '5px 10px', borderRadius: 8, border: '1px solid var(--red-bg)', color: 'var(--red)', background: 'transparent', cursor: 'pointer' }}>삭제</button>
              </div>
            </div>
          </div>
        );
      ...
      <button onClick={() => { setEditItem(null); setForm({}); setShowItemModal(true); }} style={{ width: '100%', border: '1.5px dashed var(--accent)', borderRadius: 14, padding: 14, fontSize: 14, fontWeight: 700, color: 'var(--accent)', background: 'transparent', cursor: 'pointer', marginTop: 16 }}>+ 생필품 추가</button>
        ...
        <Modal title={editItem ? '생필품 수정' : '생필품 추가'} onClose={() => { setShowItemModal(false); setEditItem(null); setForm({}); }}>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
              {DEFAULT_UNITS.map(u => (
                <button key={u} onClick={() => F('unit', form.unit===u ? '' : u)} style={{
                  padding: '6px 12px', borderRadius: 20, fontSize: 12, fontWeight: 600,
                  background: form.unit===u ? 'var(--accent)' : 'var(--bg)',
                  color: form.unit===u ? '#fff' : 'var(--sub)',
                  border: `1.5px solid ${form.unit===u ? 'var(--accent)' : 'var(--border)'}`,
                  cursor: 'pointer',
                }}>{u}</button>
              ))}
            </div>
            <input style={inp} placeholder="직접 입력" value={form.unit || ''} onChange={e => F('unit', e.target.value)} />
          </div>
          <div style={{ marginBottom: 14 }}>
          ...
          <SaveBtn onClick={saveItem} disabled={!form.name || !form.unit} />
        </Modal>
      )}
      ...
      {detailItem && (
        <ItemDetail
          ...
          onUpdateUnit={newUnit => updateUnit(detailItem.id, newUnit)}
        />
      )}
    </div>
  );
